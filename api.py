...
logging.basicConfig(filename='apilog.log', format='%(asctime)s %(levelname)s %(name)s %(threadName)s : '
...
def home():
    logging.info('Landing page request.')
...
def api_all():
    logging.info('Get games request.')
...
def api_filter_by_name():
    logging.info('Get games by team name request.')
    if 'team' in request.args:
        team = str(request.args['team']).lower()
    else:
    ...
    for game in games_in_last_24_hours:
        if team == game.get('teams').get('teamA').lower() or team == game.get('teams').get('teamB').lower():
            results.append(game)
...
def page_not_found(e):
    logging.warning('User tried to access page that does not exist.')
